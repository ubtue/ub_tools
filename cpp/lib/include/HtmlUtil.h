...
/** \brief  Replaces all HTML entities in "s" with the actual characters.
 *  \param  s                    The string that may contain optional HTML entities.
 *  \param  unknown_entity_mode  If set to REMOVE_UNKNOWN_ENTITIES, unknown entities will be replaced with the � character.
 *                               o/w "junk" will be passed through.
 *  \note   It is probably a good idea to call RemoveTags before calling this function.
 */
std::string &ReplaceEntitiesUTF8(std::string * const s, const UnknownEntityMode unknown_entity_mode = IGNORE_UNKNOWN_ENTITIES);

inline std::string ReplaceEntitiesUTF8(const std::string &s, const UnknownEntityMode unknown_entity_mode = IGNORE_UNKNOWN_ENTITIES) {
    std::string temp_s(s);
