 ...
 *
 *  The container is modified. The random sample at the end of the original container from the returned iterator to the end of the
 * container. Similar to std::remove.
 */
template <typename RandomAccessIterator>
RandomAccessIterator RandomSample(const size_t size, RandomAccessIterator first, RandomAccessIterator last) {
    static Rand rand;
...
typename RandomAccessContainer::iterator RandomSample(size_t size, RandomAccessContainer * const container) {
 ...
 */
template <typename RandomAccessContainer>
typename RandomAccessContainer::iterator RandomSample(const size_t size, RandomAccessContainer * const container,
                                                      const std::vector<double> &distribution, const unsigned seed) {
...
void RemoveRandomSample(const size_t size, std::unordered_set<Entry> * const source, std::unordered_set<Entry> * const target) {
