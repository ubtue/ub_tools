...
#define RDOUBLE(x)                                                      \
    rdoublef(x) /* Return the integer nearest x in the direction of the \
                 * prevailing rounding mode. */
#define REMAINDER(x, y)                                                    \
    remainderf(x, y) /* Return the remainder of integer divison x / y with \
                      * infinite precision. */
#define NEARBYDOUBLE(x)                                                    \
    nearbydoublef(x) /* Round x to integral value in floating-point format \
                      * using current rounding direction, but do not raise \
                      * inexact exception. */
#define ROUND(x)                                                     \
    roundf(x) /* Round x to nearest integral value, rounding halfway \
               * cases away from zero. */
#define TRUNC(x)                                                                  \
    truncf(x)                  /* Round x to the integral value in floating-point \
...
#define RDOUBLE(x)                                                          \
    std::rdouble(x) /* Return the integer nearest x in the direction of the \
                     * prevailing rounding mode. */
#define REMAINDER(x, y)                                                        \
    std::remainder(x, y) /* Return the remainder of integer divison x / y with \
                          * infinite precision. */
#define NEARBYDOUBLE(x)                                                        \
    std::nearbydouble(x) /* Round x to integral value in floating-point format \
                          * using current rounding direction, but do not raise \
                          * inexact exception. */
#define ROUND(x)                                                         \
    std::round(x) /* Round x to nearest integral value, rounding halfway \
                   * cases away from zero. */
#define TRUNC(x)                                                                      \
    std::trunc(x)                  /* Round x to the integral value in floating-point \
                                    * format nearest but not larger in magnitude. */
