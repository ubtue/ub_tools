...
public:
    ...
    AutoConvert(const AutoConvert &rhs) { *this = rhs; }

    /** \brief   Needed to use an AutoConvert as a functor.
     *
     */
    template <typename Type>
    AutoConvert &operator()(Type &item) {
    ...
    explicit AutoConvert(const double number_value): string_pointer_(string_value_) {
        StringUtil::FastFormat(string_value_, sizeof(string_value_) - 1, "%.16g", static_cast<double>(number_value));
    }
...
template <typename First = std::string, typename Second = std::string>
class AutoConvertPair {
...
public:
    ...
    std::pair<Second, First> operator()(const std::pair<First, Second> &item) {
