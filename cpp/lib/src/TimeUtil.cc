...
static std::map<std::string, float> GetTimeZoneAbbreviationMapping() {
    // The information was taken from https://www.timeanddate.com/time/zones/
    std::map<std::string, float> timezone_abbreviation;
...
static bool ZoneAdjustment(const std::string &rfc822_zone, time_t * const adjustment) {
    static std::map<std::string, float> tmz_map(GetTimeZoneAbbreviationMapping());
