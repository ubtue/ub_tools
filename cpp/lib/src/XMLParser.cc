...
bool XMLParser::getNext(XMLPart * const next, const bool combine_consecutive_characters, const std::set<std::string> &guard_tags) {
    try {
        ...
        if (not buffer_.empty()) {
            if (buffer_.front().type_ == XMLPart::OPENING_TAG and guard_tags.find(buffer_.front().data_) != guard_tags.cend())
                return false;
