...
bool ControlNumberGuesser::getNextTitle(std::string * const title, std::set<std::string> * const control_numbers) const {
    ...
    if (next_row) {
        *title = next_row["title"];
        ...
        StringUtil::Split2(concatenated_control_numbers, '|', control_numbers, /* suppress_empty_components = */true);
        return true;
    } else {
        title_cursor_.reset();
        return false;
...
std::string ControlNumberGuesser::NormaliseAuthorName(const std::string &author_name) {
    ...
    StringUtil::Split2(normalised_author_name, ' ', &parts, /* suppress_empty_components = */true);
