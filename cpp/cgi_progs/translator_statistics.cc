...
void GetTranslatorFullNames(const IniFile &ini_file, std::vector<std::string> * const rows) {
    ...
    for (auto translator_fullname_id : translator_fullname_ids)
        translator_ids_to_fullnames.emplace(translator_fullname_id,
                                            ini_file.getString(section_translator_full_names, translator_fullname_id));
    ...
    for (auto language_to_translator : languages_to_translators)
        rows->emplace_back("<tr><td>" + language_to_translator.first + "</td><td>" + StringUtil::Join(language_to_translator.second, ", ")
                           + "</td></tr>");
}


void GetSubsystemStatisticsAsHTMLRowsFromDatabase(DbConnection &db_connection, const std::string &target,
                                                  std::vector<std::string> * const rows, const std::string &start_date,
    ...
    std::string query_translated("SELECT language_code,COUNT(DISTINCT(" + id + ")) AS translated_number FROM " + table_name
    ...
    std::string query("SELECT language_code,COUNT(distinct " + id + ") AS number FROM " + table_name
                      + " WHERE next_version_id IS NULL AND prev_version_id IS NOT NULL AND create_timestamp >= '" + start_date
                      + "' AND create_timestamp <= '" + end_date + " 23:59:59" + "' GROUP BY language_code;");
...
void GetVuFindStatisticsNewEntriesFromDatabase(DbConnection &db_connection, std::string * const number_new_entries,
                                               const std::string &start_date, const std::string &end_date) {
    std::string query("SELECT token FROM vufind_translations WHERE create_timestamp>='" + start_date + "' AND create_timestamp<='"
                      + end_date + " 23:59:59"
                      + "' AND token IN (SELECT token FROM vufind_translations GROUP BY token HAVING COUNT(*)=1);");
    DbResultSet result_set(ExecSqlAndReturnResultsOrDie(query, &db_connection));
...
void GetKeyWordStatisticsNewEntriesFromDatabase(DbConnection &db_connection, std::string * const number_new_entries,
                                                const std::string &start_date, const std::string &end_date) {
    std::string query("SELECT ppn FROM keyword_translations WHERE create_timestamp>='" + start_date + "' AND create_timestamp<='" + end_date
                      + " 23:59:59" + "' AND ppn IN (SELECT ppn FROM keyword_translations GROUP BY ppn HAVING COUNT(*)=1);");
    DbResultSet result_set(ExecSqlAndReturnResultsOrDie(query, &db_connection));
    number_new_entries->clear();
    unsigned counter(0);
    while (const auto db_row = result_set.getNextRow()) { // could use result_set.size(), but prob. more information requested in web gui
...
void ShowFrontPage(DbConnection &db_connection, const IniFile &ini_file, const std::string &target, const std::string &start_date,
                   const std::string &end_date) {
