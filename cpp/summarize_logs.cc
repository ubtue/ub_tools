...
void SummarizeLog(File * const log_file, File * const summary_file) {
    ...
    RegexMatcher * const datetime_matcher(RegexMatcher::RegexMatcherFactory("^\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{]2}", &err_msg));
    if (matcher == nullptr)
    ...
    for (const auto &line_and_frequency : lines_and_frequencies)
        lines_and_frequencies_as_vector.emplace_back(std::make_pair<>(line_and_frequency.first, line_and_frequency.second));

    std::sort(lines_and_frequencies_as_vector.begin(), lines_and_frequencies_as_vector.end(), LineAndFrequencyCompare);
