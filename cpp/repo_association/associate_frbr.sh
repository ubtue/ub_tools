...
for ppn in ${repo_ppns}; do
    echo -n ${ppn}" "
    oa_link=
    work_keys_str_mv=$(curl --fail --silent ${REPO_WORK_KEYS_STR_MV_URL}${ppn} | jq '.response.docs[].work_keys_str_mv')
    # Fixme: cannot determine OA status from URL-field
    oa_link=$(curl --fail --silent ${REPO_OA_LINKS}${ppn} | jq -r '.response.docs[].urls_and_material_types[] | select(. | index("kostenfrei"))' 2>/dev/null | sed 's#:[^:]*$##')
    echo -n ${oa_link}
    query_string=$(php assemble_frbr_string.php ${ppn} "${work_keys_str_mv}")
    query_string="${SOLR_BASE_URL}fl=id&${query_string}"
    results=$(curl --fail --silent $query_string | jq -r '.response.docs[].id')
    if [ -z "${results}" ]; then
       echo
    else
