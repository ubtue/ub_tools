...
const std::string EMAIL_NOTIFICATION_SECTION("EmailNotifications");
...
std::string GetNotifyThreshold(const IniFile &ini_file, DbConnection &db_connection, const std::string &user) {
    ...
    ini_file.lookup(EMAIL_NOTIFICATION_SECTION, user, &notify_interval);
...
void NotifyTranslators(const IniFile &ini_file, DbConnection &db_connection, const bool debug = false) {
    if (ini_file.getSection(EMAIL_NOTIFICATION_SECTION) == ini_file.end())
        LOG_ERROR("No section \"" + EMAIL_NOTIFICATION_SECTION + "\" present in " + CONF_FILE_PATH);

    std::set<std::string> users;
    GetUsers(ini_file, &users);
    for (const std::string &user : users) {
        const std::string last_notified(GetLastNotified(db_connection, user));
