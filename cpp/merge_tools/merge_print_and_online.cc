...
[[noreturn]] void Usage() {
    ::Usage(
        "[--debug] [--skip-db-updates] marc_input marc_output missing_ppn_partners_list\n"
...
std::string ExtractLinkPPN(const MARC::Record::Field &field) {
    ...
    auto subfield_w(
        std::find_if(subfields.begin(), subfields.end(), [](const MARC::Subfield &subfield) -> bool { return subfield.code_ == 'w'; }));
...
void SerializeMap(const std::string &output_filename, const std::unordered_map<std::string, ValueType> &map) {
 ...
 */
void CollectRecordOffsetsAndCrosslinks(const bool debug, MARC::Reader * const marc_reader,
    ...
    while (const auto record = marc_reader->read()) {
        ...
        const std::string new_canonical_ppn(
            *std::max_element(equivalent_ppns.begin(), equivalent_ppns.end(), [](std::string a, std::string b) -> bool { return a < b; }));
...
unsigned PatchLinks(MARC::Record * const record, const std::unordered_map<std::string, std::string> &ppn_to_canonical_ppn_map) {
    ...
    for (auto field(record->begin()); field != record->end(); ++field) {
        ...
        if (std::find(MARC::CROSS_LINK_FIELD_TAGS.begin(), MARC::CROSS_LINK_FIELD_TAGS.end(), field_tag)
            != MARC::CROSS_LINK_FIELD_TAGS.end()) {
...
std::string MergeSubfieldContents(const std::string &subfield1, const std::string &subfield2, const MARC::Record &record1,
    ...
    const auto canonised_contents_without_suffix1(
        StripElectronicAndPrintSuffixes(subfield1, &electronic_or_print1, &non_canonised_contents_without_suffix1));

    std::set<ElectronicOrPrint> electronic_or_print2;
    std::string non_canonised_contents_without_suffix2;
    const auto canonised_contents_without_suffix2(
        StripElectronicAndPrintSuffixes(subfield2, &electronic_or_print2, &non_canonised_contents_without_suffix2));
...
MARC::Subfields MergeFieldContents(const MARC::Subfields &subfields1, const MARC::Record &record1, const MARC::Subfields &subfields2,
                                   const MARC::Record &record2) {
...
bool MergeFieldPairWithNonRepeatableFields(MARC::Record::Field * const merge_field, const MARC::Record::Field &import_field,
    ...
    if (merge_field->isControlField() or import_field.isControlField() or merge_field->isRepeatableField()
        or import_field.isRepeatableField())
        return false;
...
bool MergeFieldPair022(MARC::Record::Field * const merge_field, const MARC::Record::Field &import_field, MARC::Record * const merge_record,
                       const MARC::Record &import_record, MARC::Record::Field * const augmented_import_field) {
...
bool MergeFieldPair245(MARC::Record::Field * const merge_field, const MARC::Record::Field &import_field, MARC::Record * const merge_record,
                       const MARC::Record &import_record, const bool import_record_is_newer) {
    ...
    insert_field.filterSubfields(
        "abhnp68"); // See https://www.loc.gov/marc/bibliographic/bd245.html and https://www.loc.gov/marc/bibliographic/bd246.html
...
bool MergeFieldPair264(MARC::Record::Field * const merge_field, const MARC::Record::Field &import_field, MARC::Record * const merge_record,
                       const MARC::Record &import_record) {
...
void PatchSerialSubscriptions(DbConnection *connection, const std::unordered_map<std::string, std::string> &ppn_to_canonical_ppn_map) {
    for (const auto &ppn_and_ppn : ppn_to_canonical_ppn_map) {
        connection->queryOrDie(
            "SELECT user_id,max_last_modification_time FROM ixtheo_journal_subscriptions WHERE "
        ...
        while (const DbRow ppn_first_row = ppn_first_result_set.getNextRow()) {
            ...
            connection->queryOrDie(
                "SELECT max_last_modification_time FROM ixtheo_journal_subscriptions "
                "WHERE user_id='"
                + user_id + "' AND journal_control_number_or_bundle_name='" + ppn_and_ppn.second + "'");
            ...
            if (ppn_second_result_set.empty()) {
                connection->queryOrDie("UPDATE ixtheo_journal_subscriptions SET journal_control_number_or_bundle_name='"
                                       + ppn_and_ppn.second + "' WHERE user_id='" + user_id
                                       + "' AND journal_control_number_or_bundle_name='" + ppn_and_ppn.first + "'");
                continue;
            }
...
void PatchPDASubscriptions(DbConnection *connection, const std::unordered_map<std::string, std::string> &ppn_to_canonical_ppn_map) {
    for (const auto &ppn_and_ppn : ppn_to_canonical_ppn_map) {
        ...
        while (const DbRow row = result_set.getNextRow())
            connection->queryOrDie("UPDATE ixtheo_pda_subscriptions SET book_ppn='" + ppn_and_ppn.first + "' WHERE id='" + row["id"]
                                   + "' AND book_ppn='" + ppn_and_ppn.second + "'");
    }
}
...
int Main(int argc, char *argv[]) {
    ...
    CollectRecordOffsetsAndCrosslinks(debug, marc_reader.get(), &ppn_to_offset_map, &ppn_to_canonical_ppn_map, &canonical_ppn_to_ppn_map);

    EliminateDanglingOrUnreferencedCrossLinks(debug, ppn_to_offset_map, &ppn_to_canonical_ppn_map, &canonical_ppn_to_ppn_map);

    marc_reader->rewind();
    MergeRecordsAndPatchLinks(marc_reader.get(), marc_writer.get(), ppn_to_offset_map, ppn_to_canonical_ppn_map, canonical_ppn_to_ppn_map);
