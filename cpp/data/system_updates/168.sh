...
TOKEN=$(inifile_lookup "/usr/local/var/lib/tuelib/Elasticsearch.conf" Elasticsearch token)
...
if [ $? -eq 0 ]; then
  ...
  username=$(inifile_lookup "/usr/local/var/lib/tuelib/Elasticsearch.conf" Elasticsearch username)
  password=$(inifile_lookup "/usr/local/var/lib/tuelib/Elasticsearch.conf" Elasticsearch password)
