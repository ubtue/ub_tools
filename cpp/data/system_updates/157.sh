...
if [[ $TUEFIND_FLAVOUR == "ixtheo" ]]; then
    ...
    sed --in-place 's|20 23 \* \* \* "\$BIN/rss_aggregator" "--use-web-proxy" "ixtheo" "\$EMAIL" "/usr/local/vufind/public/docs/news.rss" > "\$LOG_DIR/rss_aggregator_ixtheo.log" 2>\&1|30 23 * * * "$BIN/rss_aggregator" "ixtheo" "$EMAIL" "/usr/local/vufind/public/docs/news.rss" > "$LOG_DIR/rss_aggregator_ixtheo.log" 2>\&1|' $root_crontab
fi