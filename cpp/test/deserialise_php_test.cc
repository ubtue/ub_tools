...
void PrintString(const std::shared_ptr<PHPUtil::DataType> &string_candidate, const unsigned indent) {
    ...
    if (unlikely(string == nullptr))
        logger->error("in PrintString: expected PHPUtil::String, found " + std::string(typeid(string_candidate).name()) + " instead!");
...
void PrintInteger(const std::shared_ptr<PHPUtil::DataType> &integer_candidate, const unsigned indent) {
    ...
    if (unlikely(integer == nullptr))
        logger->error("in PrintInteger: expected PHPUtil::Integer, found " + std::string(typeid(integer_candidate).name()) + " instead!");
...
void PrintFloat(const std::shared_ptr<PHPUtil::DataType> &float_candidate, const unsigned indent) {
    ...
    if (unlikely(flt == nullptr))
        logger->error("in PrintFloat: expected PHPUtil::Float, found " + std::string(typeid(float_candidate).name()) + " instead!");
...
void PrintArray(const std::shared_ptr<PHPUtil::DataType> &array_candidate, const unsigned indent) {
    ...
    if (unlikely(array == nullptr))
        logger->error("in PrintArray: expected PHPUtil::Array, found " + std::string(typeid(array_candidate).name()) + " instead!");
...
void PrintObject(const std::shared_ptr<PHPUtil::DataType> &object_candidate, const unsigned indent = 0) {
    ...
    if (unlikely(object == nullptr))
        logger->error("in PrintObject: expected PHPUtil::Object, found " + std::string(typeid(object_candidate).name()) + " instead!");
...
int main(int argc, char *argv[]) {
    ...
    try {
        std::shared_ptr<PHPUtil::DataType> php_object(PHPUtil::DeserialisePHPObject(std::string(buffer, actual_size)));
