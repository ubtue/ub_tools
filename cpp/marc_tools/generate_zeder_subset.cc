...
std::unordered_set<std::string> GetMatchingJournalPPNs(const Zeder::Flavour zeder_flavour, const std::string &match_column,
    ...
    LOG_INFO("Processed " + std::to_string(journal_count) + " Zeder journal(s) and found " + std::to_string(match_count)
             + " matching journal(s) from which " + std::to_string(matching_journal_ppns.size()) + " PPN's were extracted!");

    return matching_journal_ppns;
}


void ProcessRecords(const bool filter_on_zdb_numbers, const bool select_articles_only, const std::unordered_set<std::string> &journal_ppns,
                    const std::unordered_set<std::string> &journal_zdb_numbers, MARC::Reader * const marc_reader,
    ...
    LOG_INFO("Processed " + std::to_string(total_record_count) + " record(s) of which " + std::to_string(matched_record_count)
             + " record(s) matched.");
}
...
[[noreturn]] void Usage() {
    ::Usage(
        "[--filter-on-zdb-numbers] [--select-articles-only] zeder_flavour match_column column_regex marc_input marc_output\n\n"
...
int Main(int argc, char *argv[]) {
    ...
    const auto matching_journal_ppns(GetMatchingJournalPPNs(zeder_flavour, match_column, regex_matcher, &matching_journal_zdb_numbers));
    delete regex_matcher;
    ProcessRecords(filter_on_zdb_numbers, select_articles_only, matching_journal_ppns, matching_journal_zdb_numbers, marc_reader.get(),
                   marc_writer.get());
