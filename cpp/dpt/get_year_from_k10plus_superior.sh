...
for ppn in $(marc_grep ${dpt_marc_file}  '"773w"' | awk -F ':' '{print $3}' | sed -re 's/^[(]DE-627[)]//' \
    ...
    echo $ppn:$(curl --fail --silent "https://sru.k10plus.de/opac-de-627?version=1.1&operation=searchRetrieve&query=pica.ppn%3D"$ppn"&maximumRecords=1&recordSchema=dc" | \
