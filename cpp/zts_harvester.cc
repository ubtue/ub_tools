                                  ...
                                  Zotero::SiteParams * const site_params)
{
    ...
    for (const auto &entry : section) {
        ...
        else if (StringUtil::StartsWith(entry.name_, "exclude_field_")) {
            const auto field_name(entry.name_.substr(__builtin_strlen("exclude_field_")));
            if (field_name.length() != MARC::Record::TAG_LENGTH && field_name.length() != MARC::Record::TAG_LENGTH + 1)
                LOG_ERROR("invalid exclusion field '" + field_name + "'! expected format: <tag> or <tag><subfield_code>");
