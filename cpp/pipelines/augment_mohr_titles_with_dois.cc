...
void ProcessRecords(MARC::Reader * const marc_reader, MARC::Writer * const marc_writer,
    ...
    LOG_INFO("Inserted " + std::to_string(new_electronic_dois) + " new electronic DOIs and " + std::to_string(new_print_dois)
             + " print DOIs of " + std::to_string(count) + " records altogether");
}
...
int Main(int argc, char **argv) {
    ...
    marc_reader->rewind();
    ProcessRecords(marc_reader.get(), marc_writer.get(), print_ppn_to_isbn_map, electronic_ppn_to_isbn_map, mohr_isbn_to_doi_map);
