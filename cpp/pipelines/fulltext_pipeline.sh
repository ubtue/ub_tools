...
(find /usr/local/publisher_fulltexts/mohr/ -name '*.txt' | xargs -n 50 store_in_elasticsearch \
    >> "${log}" 2>&1 && \
EndPhase || Abort) &


StartPhase "Import Brill Data"
(find /usr/local/publisher_fulltexts/brill/ -name '*.txt' | xargs -n 50 store_in_elasticsearch \
    >> "${log}" 2>&1 && \
EndPhase || Abort) &
