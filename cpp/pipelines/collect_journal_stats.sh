...
curl --verbose --fail --request POST --header "Content-Type: multipart/form-data" --form Datenquelle=$(hostname) --form Systemtyp=${system_type} --form "Datei=@${json_out_file}" --form "s_stufe=2" "http://www-ub.ub.uni-tuebingen.de/zeder/cgi-bin/index.cgi/artikelliste_hochladen"
rm -f "${json_out_file}"
