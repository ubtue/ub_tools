...
std::cerr << "regex_string: " << regex_string << '\n';
...
std::cerr << "tag: " << tag << " subfield_code: " << subfield_code << '\n';
...
std::cerr << "Processing " << record.getControlNumber() << '\n';
        ++total_count;
...
std::cerr << "Entering FilterType::REMOVE_SUBFIELDS" << '\n';
               ...
               if (MatchedSubfield(record, filter.getCompiledPatterns(), &matched_field_indices_and_subfields)) {
                   ...
                   modified_record = true;
std::cerr << "Modified record " << record.getControlNumber() << '\n';
                   break;
               }
           } else {
...
std::cerr << "Entering --remove-subfields branch" << '\n';
}
        else if (std::strcmp(*argv, "--remove-subfields") == 1) {
