...
if len(sys.argv) != 3:
    print(f"Usage: python {sys.argv[0]} <input_xml> <output_json>")
    sys.exit(1)
...
for record in root.findall('.//marc:record', namespaces=ns):
    for df in record.findall('marc:datafield', namespaces=ns):
        if df.attrib.get("tag") == "856":
            for sub in df.findall('marc:subfield', namespaces=ns):
                if sub.attrib.get("code") == "u":
                    url = sub.text.strip()
                    if url:
