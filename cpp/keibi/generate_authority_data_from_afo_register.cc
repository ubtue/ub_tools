...
struct AfOEntry {
    ...
    AfOEntry(const unsigned &entry_num, const std::string &keyword, const std::string &internal_reference_keyword,
             const std::string &literature_reference, const std::string &comment)
        : entry_num_(entry_num), keyword_(keyword), internal_reference_keyword_(internal_reference_keyword),
          literature_reference_(literature_reference), comment_(comment) { }
    ...
    std::string toString() const {
        return std::to_string(entry_num_) + " AAA " + keyword_ + " BBB " + internal_reference_keyword_ + " CCC " + literature_reference_
               + " DDD " + comment_;
    }
    __attribute__((unused)) friend std::ostream &operator<<(std::ostream &output, const AfOEntry &entry);
};
...
template <>
struct hash<AfOEntry> {
    inline size_t operator()(const AfOEntry &afo_entry) const { return hash<std::string>()(afo_entry.keyword_); }
};
