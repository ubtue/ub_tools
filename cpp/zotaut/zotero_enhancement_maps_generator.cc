...
[[noreturn]] void Usage() {
    ::Usage(
        "[options] [zotero_enhancement_maps_directory]\n"
        "\n"
        "\tOptions:\n"
        "\t[--min-log-level=log_level]          Possible log levels are ERROR, WARNING (default), INFO and DEBUG\n"
        "\t[zotero_enhancement_maps_directory]  or default "
        + UBTools::GetTuelibPath() + "zotero-enhancement-maps" + "\n");
}
...
void DownloadZederInstanceEntries(const Zeder::Flavour flavour,
    ...
    if (not downloader->download(&downloaded_flavour_entries, /*disable_cache_mechanism=*/true))
...
std::unordered_set<std::string> GetZederEntryISSNs(const Zeder::Entry &entry, const Zeder::Flavour flavour) {
    ...
    std::unordered_set<ZoteroHarvester::Config::JournalParams::IniKey> ini_keys{
        ZoteroHarvester::Config::JournalParams::IniKey::ONLINE_ISSN, ZoteroHarvester::Config::JournalParams::IniKey::PRINT_ISSN
    };
    for (const auto ini_key : ini_keys) {
        const std::string issn(ZoteroHarvester::ZederInterop::GetJournalParamsIniValueFromZederEntry(entry, flavour, ini_key));

        if (not issn.empty()) {
            if (MiscUtil::IsPossibleISSN(issn))
                issns.emplace(issn);
            else {
                LOG_WARNING("Skipping invalid ISSN: " + issn + " (Zeder ID: " + std::to_string(entry.getId())
...
void GenerateIssnToLicenseMap(const std::string &zotero_enhancement_maps_directory,
    ...
    for (const auto &[flavour, downloaded_flavour_entries] : downloaded_entries) {
        for (const auto &downloaded_flavour_entry : downloaded_flavour_entries) {
            std::string license(ZoteroHarvester::ZederInterop::GetJournalParamsIniValueFromZederEntry(
                downloaded_flavour_entry, flavour, ZoteroHarvester::Config::JournalParams::IniKey::LICENSE));

            if (not license.empty()) {
                StringUtil::ASCIIToLower(&license);
                const std::string name(ZoteroHarvester::ZederInterop::GetJournalParamsIniValueFromZederEntry(
                    downloaded_flavour_entry, flavour, ZoteroHarvester::Config::JournalParams::IniKey::NAME));
