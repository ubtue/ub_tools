...
void InstallUBTools(const bool make_install) {
    ...
    ExecOrDie(ExecUtil::Which("make"), { "-j4", "install" });
    ...
    if (make_install)
        ...
        ExecOrDie(ExecUtil::Which("make"), { "-j4" });
...
void InstallCronjobs(const VuFindSystemType vufind_system_type) {
    ...
    ExecOrDie(ExecUtil::Which("crontab"), { crontab_temp_file_new.getFilePath() });
...
void CreateUserIfNotExists(const std::string &username) {
    ...
    if (user_exists == 1) {
        ...
        ExecOrDie(ExecUtil::Which("useradd"), { "--system", "--user-group", "--no-create-home", username });
...
void GitAssumeUnchanged(const std::string &filename) {
    ...
    ExecOrDie(ExecUtil::Which("git"), { "update-index", "--assume-unchanged", filename });
}
...
void GitCheckout(const std::string &filename) {
    ...
    FileUtil::DirnameAndBasename(filename, &dirname, &basename);
    TemporaryChDir tmp(dirname);
    ExecOrDie(ExecUtil::Which("git"), { "checkout", filename });
}
...
void DownloadVuFind() {
    if (FileUtil::IsDirectory(VUFIND_DIRECTORY)) {
        ...
        ExecOrDie(ExecUtil::Which("git"), { "clone", git_url, VUFIND_DIRECTORY });
        ...
        ExecOrDie(ExecUtil::Which("composer"), { "install" });
    }
}
...
void ConfigureApacheUser(const OSSystemType os_system_type) {
    ...
    ExecOrDie(ExecUtil::Which("find"), { VUFIND_DIRECTORY + "/local", "-name", "cache", "-exec", "chown", "-R", username + ":" + username, "{}", "+" });
    ExecOrDie(ExecUtil::Which("chown"), { "-R", username + ":" + username, "/usr/local/var/log/tuefind" });
...
static void InstallVuFindServiceTemplate(const VuFindSystemType system_type) {
        ...
        ExecOrDie(ExecUtil::Which("mkdir"), { "-p", SYSTEMD_SERVICE_DIRECTORY });
        std::map<std::string, std::vector<std::string>> names_to_values_map
...
void ConfigureSolrUserAndService(const VuFindSystemType system_type, const bool install_systemctl) {
    ...
    ExecOrDie(ExecUtil::Which("chown"), { "-R", USER_AND_GROUP_NAME + ":" + USER_AND_GROUP_NAME, VUFIND_DIRECTORY + "/solr" });
    ExecOrDie(ExecUtil::Which("chown"), { "-R", USER_AND_GROUP_NAME + ":" + USER_AND_GROUP_NAME, VUFIND_DIRECTORY + "/import" });
    ...
    if (install_systemctl) {
        ...
        ExecOrDie(ExecUtil::Which("systemctl"), { "enable", SERVICENAME });
        ExecOrDie(ExecUtil::Which("systemctl"), { "daemon-reload" });
        ExecOrDie(ExecUtil::Which("systemctl"), { "restart", SERVICENAME });
    }
}
...
void ConfigureVuFind(const VuFindSystemType vufind_system_type, const OSSystemType os_system_type, const bool install_cronjobs, const bool install_systemctl) {
    ...
    ExecOrDie(ExecUtil::Which("mkdir"), { "-p", "/usr/local/var/lib/tuelib" });
    ExecOrDie(ExecUtil::Which("mkdir"), { "-p", "/usr/local/var/log/tuefind" });
    if (SELinuxUtil::IsEnabled()) {
