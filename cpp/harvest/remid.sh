...
declare -r FILE_TEMP1="remid_tmp1.xml"
...
# maximumRecords is limited to 1000. Since we have about 1300 datasets,
# we need to download the data in multiple steps and merge them together afterwards
declare -r RECORDS_PER_CALL=1000
startRecord=1
wget "http://sru.hebis.de/sru/DB=2.1?query=pica.abr+%3D+%22REMID%22+or+pica.prv+%3D+%22REMID%22&version=1.1&operation=searchRetrieve&recordSchema=marc21&maximumRecords=${RECORDS_PER_CALL}&startRecord=${startRecord}&recordPacking=xml&sortKeys=LST_Y%2Cpica%2C0%2C%2C" -O $FILE_ORI
