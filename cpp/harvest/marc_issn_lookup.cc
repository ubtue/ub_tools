...
[[noreturn]] void Usage() {
    ::Usage(
        ...
        "\n"
        "- marc_input_artiles is a file containing all article information taken from CORE. \n"
...
struct SubFieldInfo {
    ...
    void Builder(MARC::Record &record) {
        for (auto &field : record) {
            if (field.getTag() == "001")
                this->w = "(DE-627)" + field.getContents();

            if (field.getTag() == "773")
                this->x = field.getContents();
...
SubFieldInfo LookupISSNInDataSouce(const std::string input_journal_filename, const std::string issn) {
    auto data_source(MARC::Reader::Factory(input_journal_filename));
...
void UpdateSubfield(MARC::Subfields &subfields, const SubFieldInfo &sub_field_info) {
    ...
    if (!subfields.replaceFirstSubfield('w', sub_field_info.w))
        subfields.addSubfield('w', sub_field_info.w);
    ...
    while (MARC::Record record = input_file->read()) {
        ...
        for (auto &field : record) {
            if (field.getTag() == "773") {
                ...
                if (issn != "") {
                    SubFieldInfo sub_field_info(LookupISSNInDataSouce(argv[2], issn));
                    if (sub_field_info.is_found) {
                        MARC::Subfields subfields(field.getSubfields());

                        // update subfield
                        UpdateSubfield(subfields, sub_field_info);
