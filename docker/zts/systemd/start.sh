...
# We use journald as log driver because it's the easiest way to redirect output to a single known file
...
# (Also tried syslog, but that is not compatible with podman-docker under CentOS 8, so we use journald which redirects to syslog)
DOCKER_CMD="docker run --publish 1969:1969 --log-driver journald"
if [ -n "${ZTS_PROXY}" ]; then
