<?php
    ...
    $action = (isset($_POST['action'])) ? $_POST['action'] : 'crawling';
...
<ul class="nav nav-tabs" id="myTab" role="tablist">
    <li class="nav-item"><a class="nav-link <?=($action == 'crawling') ? 'active' : ''?>" id="home-crawling" data-toggle="tab" href="#crawling" role="tab" aria-controls="crawling" aria-selected="true">Crawling</a></li>
    <li class="nav-item"><a class="nav-link <?=($action == 'rss') ? 'active' : ''?>" id="home-rss" data-toggle="tab" href="#rss" role="tab" aria-controls="crawling" aria-selected="false">RSS</a></li>
</ul>

<!-- tab content -->
<div class="tab-content">
    <div class="tab-pane <?=($action == 'crawling') ? 'active' : ''?>" id="crawling" role="tabpanel" aria-labelledby="crawling-tab">
        <form method="post" action="index.php">
            <table>
                ...
                <tr>
                    ...
                    <td>MARC21 and MARCXML are custom. All others are generated by Zotero (JSON = native Zotero API format).</td>
                </tr>
    ...
    <div class="tab-pane <?=($action == 'rss') ? 'active' : ''?>" id="rss" role="tabpanel" aria-labelledby="rss-tab">
        <form method="post" action="index.php">
            <table>
                <tr><td>RSS Feed Url</td><td><input name="urlRss" type="text" size="60" value="<?= isset($_POST['urlRss']) ? $_POST['urlRss'] : 'https://ipfs.io/blog/index.xml' ?>"></input></td><td>e.g. https://ipfs.io/blog/index.xml</td></tr>
                <tr>
                    <td>Format</td>
                    <td>
                        <select name="outputFormat">
                            <?php
                                foreach (\RSS\MetadataHarvester::OUTPUT_FORMATS as $format_key => $format_extension) {
                                    print '<option value="'.$format_key.'" ';
                                    if (isset($_POST['outputFormat']) && $_POST['outputFormat'] == $format_key) print 'selected';
                                    print '>' . mb_strtoupper($format_key) . '</option>';
                                }
        ...
        <?php
            if ($action == 'rss' && isset($_POST['urlRss'])) {
                $rss_harvester = new RSS\MetadataHarvester(ZOTERO_TRANSLATION_SERVER_URL);
                $result = $rss_harvester->start($_POST['urlRss'], $_POST['outputFormat']);
                ?>

                <h2>Result</h2>
                <table>
                    <tr><td>Command</td><td><?= $result->cmd ?></td></tr>
                    <?php
                        if ($result->exitCode == 0) {
                            print '<tr><td>Download</td><td><a target="_blank" href="getresult.php?id=' . basename($result->outPath) . '">Result file</a></td></tr>';
                        } else {
                            print '<tr><td>ERROR</td><td>Exitcode: '.$status['exitcode'].'</td></tr>';
                        }
                        print '<tr><td>CLI output:</td><td>'.nl2br(htmlspecialchars($result->output)).'</td></tr>';
                    ?>
                </table>
                <?php
            }
