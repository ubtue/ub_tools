       ...
       // Support for Lucene parser
           ...
           if (query.length == 1) {
               try {
                        ...
                        while (boolIterator.hasNext()) {
                            ...
                            if (termQueryCandidate instanceof TermQuery){
                                ...
                                if (schema.getFieldOrNull(newFieldName) != null) {
                                    termQueryCandidate = new TermQuery(new Term(newFieldName, termQuery.getTerm().text()));
                                }
                            }
                        }
                  }
                  ...
                  // Must be explicitly called otherwise we get the cached version from getQuery() above
                  parse();
...
System.err.println("CALLED PARSE WITH: " +  this.searchString);
System.err.println("NEW PARAMS: " + newParams.get("q"));
