...
public class PerSegmentSingleValuedFaceting {
    ...
    public PerSegmentSingleValuedFaceting(SolrIndexSearcher searcher, DocSet docs, String fieldName, int offset,
            int limit, int mincount, boolean missing, String sort, String prefix) {
    ...
    NamedList<Integer> getFacetCounts(Executor executor) throws IOException {
        ...
        // TODO: Is there a completion service, or a delegating executor that
        // can
        ...
        if (missing) {
            if (!hasMissingCount) {
                missingCount = SimplePrefixSortFacets.getFieldMissingCount(searcher, docs, fieldName);
            }
    ...
    class SegFacet {
        ...
        void countTerms() throws IOException {
            ...
            // SolrCore.log.info("reader= " + reader + " FC=" +
            // System.identityHashCode(si));
            ...
            if (prefix != null) {
                ...
                // TODO: we could constrain the lower endpoint if we had a
                // binarySearch method that allowed passing start/end
            ...
            if (nTerms > 0) {
                // count collection array only needs to be as big as the number
                // of terms we are
                // going to collect counts for.
                final int[] counts = this.counts = new int[nTerms];
                DocIdSet idSet = baseSet.getDocIdSet(context, null); // this set
                                                                     // only
                                                                     // includes
                                                                     // live
                                                                     // docs
                ...
                if (prefix == null) {
                    ...
                    // version that adjusts term numbers because we aren't
                    // collecting the full range
                    while ((doc = iter.nextDoc()) < DocIdSetIterator.NO_MORE_DOCS) {
...
class CountSortedFacetCollector extends FacetCollector {
    ...
    final int limit;
    final int maxsize;
    final BoundedTreeSet<SimplePrefixSortFacets.CountPair<String, Integer>> queue;
    ...
    public boolean collect(BytesRef term, int count) {
        if (count > min) {
            ...
            queue.add(new SimplePrefixSortFacets.CountPair<>(spare.toString(), count));
    ...
    public NamedList<Integer> getFacetCounts() {
        ...
        for (SimplePrefixSortFacets.CountPair<String, Integer> p : queue) {
            if (--off >= 0)
                continue;
            if (--lim < 0)
                break;
