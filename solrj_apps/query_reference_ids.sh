...
cat $reffile | awk --file rewrite_query_expression.awk | xargs -t -0 --max-args=2 --max-procs=8 java -cp $CLASSPATH  $QUERYREF_MAIN $outputdir

# Create a with with a list of IDs and all matching reference terms
cat $outputdir/* | sort -k1 > $outputdir/UNIFIED
